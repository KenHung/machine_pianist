...
class MachinePianist:
    ...
    def perform_midis(self, midi_files: list):
        ...
        X = generate_song_tensors(
            songs_list=preprocessed_dfs,
            solutions=False,
        ...
        print(
            "[INFO] Machine Pianist - Songs performed! Playtime: %.2f seconds."
            % (time.time() - start_time)
        )

        # Get the postprocessed songs and return them.
        return generate_output_midi(
            preprocessed_songs,
            Y_hat,
            X,
            scaler_X=self._scaler_X,
...
if __name__ == "__main__":
    ...
    if args.g is True:
        for midi_path in midi_files:
            ...
            if args.d is True:
                print_first_x(midi, 50, notes_only=False)
