...
class PianoPlayer:
    ...
    def play_mido(self, midi: MidiFile, play_for=None, block=False, verbose=True):
        ...
        self.local_load_and_play(
            self._mido_temp_file, play_for=play_for, block=block, verbose=verbose
        )
    ...
    def local_load_and_play(self, location, play_for=None, block=False, verbose=True):
        ...
        try:
            ...
            if block or (play_for is not None and play_for > 0):
                ...
                if verbose:
                    print("[DEBUG] PianoPlayer blocking...")
                while pygame.mixer.music.get_busy() and (
                    play_for is None or play_for > 0
                ):
                ...
                if verbose:
                    print(
                        "[DEBUG] PianoPlayer completed in %.2f seconds."
                        % (time.time() - start_time)
                    )
        except Exception as e:
            print(
                "[ERROR] PianoPlayer was unable to locally play song from location '"
                + str(location)
                + "'. Exception: "
            )
            print(e)
...
def midi_statistics(mid):
    ...
    for i, track in enumerate(mid.tracks):
        print(
            "[INFO] Midi Player - Track {}: {} - Length: {}".format(
                i, track.name, str(len(track))
            )
        )
...
def graph_controls_notes(mid):
    ...
    plt.legend(
        ["Song Notes", "Sustain Control (64)", "Soft Control (67)"], loc="upper left"
    )
    plt.show()
